...
| `destroy()` cleanup contract                         | README/API promises                                             | `packages/canvist/src/editor.ts` (`destroy()` aborts AbortController, removes DOM elements and ARIA attrs, frees WASM)                                                           | ⚠️     | Implementation reviewed; browser-level GC assertion needs Playwright harness.                         |
